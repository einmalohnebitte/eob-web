...
export const ShopsMap: React.FC<{
  ...
  shops: GatsbyTypes.TownsShopsCatQuery["allShop"]["edges"];
  width?: string;
  height?: string;
    ...
    >
      ...
      <MarkerClusterGroup>
        {(shops ?? []).map(
          (item, k) =>
            item.node.points && (
              <Marker key={k} position={item.node.points as [number, number]}>
                <Popup>
                  <b>{item.node.name}</b>
                  <p>{item.node.oeffnungszeiten}</p>
                  <p>
                    {item.node.strasse} - {item?.node?.stadt?.name}
                  </p>
                  <p>{(item.node.categories ?? []).join(" - ")}</p>
                </Popup>
              </Marker>
            )
        )}
